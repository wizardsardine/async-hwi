...
impl<T: Transport + Sync + Send> Jade<T> {
    ...
    pub async fn auth(&self) -> Result<(), JadeError> {
        ...
        if let api::AuthUserResponse::PinServerRequired { http_request } = res {
            ...
            let handshake_init_params: api::HandShakeInitParams =
                client.request(http_request.params).await?;
            let handshake: api::HandShakeInitResponse = self
                .transport
                .request("handshake_init", Some(handshake_init_params))
                .await?
                .into_result()?;
            let handshake_completed_params: api::HandShakeCompleteParams =
                client.request(handshake.http_request.params).await?;
            let handshake_completed: bool = self
                .transport
                .request("handshake_complete", Some(handshake_completed_params))
                .await?
                .into_result()?;
...
async fn read_stream<D: DeserializeOwned, S: AsyncRead + Unpin>(
    ...
    Err(TransportError::NoErrorOrResult)
}
